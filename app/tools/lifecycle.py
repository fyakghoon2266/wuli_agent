...
def extract_markdown_table(table) -> str:
    """將 BeautifulSoup 的 table 物件轉換為 Markdown 格式的字串"""
    extracted_data = []
...
def parse_aws_bedrock_tables(html_content: str) -> str:
    """精準解析 AWS Bedrock Lifecycle 網頁的 Legacy 與 EOL 表格"""
...
def parse_azure_openai_tables(html_content: str) -> str:
    """精準解析 Azure OpenAI 的 Current models 與 Fine-tuned models 表格"""
    soup = BeautifulSoup(html_content, 'html.parser')
    extracted_data = []

    # 1. 抓取「Current models (包含所有 Tab)」區塊
    ...
    if current_models_heading:
        ...
        # Azure 的多個 tabpanel 都包在 tabGroup 裡面
        ...
        if tab_group:
            ...
            # 如果沒有 tabGroup，就抓下一個 table
            table = current_models_heading.find_next('table')
            if table:
                extracted_data.append(extract_markdown_table(table))

    # 2. 抓取「Fine-tuned models」區塊
