...
class SendPhotoAlbumSkill(SkillBase):
    name = "send_photo_album"
    description = "Wuli / Milu 照片彩蛋"
    permission = "user"
    tags = ["easter_egg", "photo", "fun"]
    status_message = "📸 Wuli 正在翻找相簿..."
    sop = (
        "5. **📸 相簿彩蛋工具 (Photo Album)**：\n"
        "   - 當使用者想看你的照片 (Wuli 的自拍)，或是明確要求想看你妹妹 (Milu / 咪嚕) 的照片時，"
        "請務必呼叫 `send_photo_album` 工具。\n"
        "   - 你必須透過傳入的 `query` 參數，告訴工具使用者想看的是誰 "
        "(例如傳入 \"wuli\" 或 \"想看妹妹\")。\n"
    )
